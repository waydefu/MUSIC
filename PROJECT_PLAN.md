...
而五個虛擬喇叭送到兩耳的貢獻轉回 M/S 之後，交叉項會整組消掉，只剩：
...
也就是**每格四次複數乘法**，然後照舊兩次 irfft —— 與 Basic Stereo Renderer
...
| | full（P1 stereo） | full --binaural（P2 HRTF） |
|---|---|---|
| mean | 7.867 ms / 13.11% | 7.671 ms / **12.78%** |
| p50 | 7.016 ms / 11.69% | 6.178 ms / 10.30% |
| p99 | 17.753 ms / 29.59% | 18.441 ms / 30.74% |
| max | 22.047 ms / 36.74% | 27.171 ms / 45.29% |

**HRTF 的成本落在本機的跑次間變異之內。** mean 甚至量到比 stereo 低 0.33 個
百分點 —— 那不可能是真的變快，只代表訊號被雜訊蓋過。同一台機器上 §9.8 記的
full mean 是 12.49%，這次 stereo 量到 13.11%，光跑次差異就 0.6 個百分點，
比 binaural 的差值還大。offline 的配對 p50 量測（四組）落在 −0.22 ~ +0.17 ms，
同樣蓋不過雜訊。
...
* [src/aurora/core/hrtf.py](src/aurora/core/hrtf.py)：合成球形頭模型
  （Woodworth ITD + Brown–Duda 遮蔽），13 條測試。
* `SpatialUpmix.binaural`：**預設關閉**，關著時一格濾波器都不算，
  回呼成本與 P1 相同。場景建構完全沒動，換掉的只有 renderer。

#### 已知缺口（下一個切片）

1. **全開時耳間相關性（IACC）會變成負的。** 環繞是以 ±u 反相餵給 SL/SR 的：
   P1 折回立體聲時無害，經過 HRTF 之後會產生反相音場。實測 amount=1.0 時
   IACC = −0.45，而真實雙耳渲染不會讓一般音樂變成負相關；0.5 以下兩個
   renderer 幾乎重疊（0.623 vs 0.632）。已用 strict xfail 釘住，修好的那天
   測試會因為「意外通過」而叫。修法（餵獨立的去相關訊號、或補 diffuse-field
   EQ）都要動到場景建構。
2. **沒有耳廓 ⇒ 沒有可靠的前後區分，也沒有真正的頭外化。** 合成模型只給
   ITD 與 ILD。SADIE II（Apache-2.0，**不進版控**）是下一步，介面就是
   `HrtfFilters.from_ear_pairs`。UI 上不能拿合成模型冒充 HRTF 完成品。
3. **UI 尚未接線**，沒有使用者可見的開關。
4. **打包版不含此變更**：最近一次 `build_exe.py --verify` 的狀態是
...
6. 🔨 P2 HRTF —— renderer 骨架與預算重測完成（§9.10）；還缺真人資料集、
   環繞反相的修正、UI 接線
