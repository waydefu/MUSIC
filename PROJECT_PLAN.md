...
### 9.10 P2 HRTF：renderer 骨架與預算重測（VERIFIED）

§9.9 寫明「P2 HRTF 開始之前」要重新檢視預算。這是那次重測，結論比 §9.4
當初的預期好很多。

#### 架構：不只共用 STFT，連 FFT 次數都沒有增加

§9.4 的結論是「必須與 Spatial 共用同一個 STFT」。實際做下去發現可以更省。
場景是 M/S 表示（`_build_scene` 回傳 centre／front_mid／surround_side），
而一對對稱喇叭餵 `(a, b)` 時，兩耳的貢獻轉回 M/S 之後交叉項會整組消掉，
只剩一條通用規則 —— **餵法的和進 mid、差進 side**：

```
M ← (a+b)/2 · H_sum(θ)        H_sum(θ)  = H_i(θ) + H_c(θ)
S ← (a−b)/2 · H_diff(θ)       H_diff(θ) = H_i(θ) − H_c(θ)
```

代進場景（環繞餵兩條去相關訊號 D₁、D₂，理由見下面「環繞不能餵 ±u」）：
...
也就是**每格五次複數乘法**，然後照舊兩次 irfft —— 與 Basic Stereo Renderer
完全相同的 FFT 次數。推導寫在 [src/aurora/core/hrtf.py](src/aurora/core/hrtf.py)
的 module docstring，並由 [tests/test_hrtf.py](tests/test_hrtf.py) 拿**不共用任何
化簡**的逐喇叭參考實作驗證等價。

前提是**場景左右對稱**。之後做 re-panning 讓個別物件不對稱時，這條捷徑要重推。

#### 量測（`--device`，2000 次回呼，全鏈最大設定）

兩次 session，每次都當場量一組 stereo 當基線：

| | stereo | binaural | | stereo | binaural |
|---|---|---|---|---|---|
| | **① 四條濾波器** | | | **② 五條 + 去相關環繞** | |
| mean | 13.11% | **12.78%** | | 14.01% | **13.15%** |
| p50 | 11.69% | 10.30% | | 11.38% | 11.56% |
| p99 | 29.59% | 30.74% | | 33.97% | 28.13% |

**HRTF 的成本落在本機的量測解析度之下。** 兩次 session 的 binaural mean 都量到
比同場的 stereo **低** —— 那不可能是真的變快，只代表訊號被雜訊蓋過。更直接的
證據是 stereo 基線自己在兩次之間就差了 0.9 個百分點（13.11% → 14.01%），
那已經大於要量的東西。p50 的差值兩次連正負號都不一致。

offline 的配對 p50 量測（四組）落在 −0.22 ~ +0.17 ms，結論相同。

重點是它**沒有**像 §9.4 擔心的那樣再加一組 STFT —— 那會讓 Spatial 的成本翻倍
（約 +2.8 ms ≈ 4.7 個百分點），那個量級不可能被 0.9 個百分點的雜訊藏住。

重點是它**沒有**像 §9.4 擔心的那樣再加一組 STFT —— 那會讓 Spatial 的成本翻倍
（約 +2.8 ms），那個量級不可能被雜訊藏住。

> **§9.9 的預算決定不因為 P2 HRTF 而改變。** 超標的仍然只有全鏈最大設定，
> 幅度也沒有變深。

註：2000 次回呼對 p99 只有 20 個尾端樣本。章程 §6.3 談 p99.9 要數千次，
所以這裡只有 mean 與 p50 可用，p99／max 當量級參考。

#### 目前做到哪裡

* [src/aurora/core/hrtf.py](src/aurora/core/hrtf.py)：合成球形頭模型
  （Woodworth ITD + Brown–Duda 遮蔽）。13 條模型測試 + 11 條 renderer 測試。
* `SpatialUpmix.binaural`：**預設關閉**，關著時一格濾波器都不算，
  回呼成本與 P1 相同。場景建構完全沒動，換掉的只有 renderer。

#### 環繞不能餵 ±u（已修）

第一版的 renderer 沿用 P1 折回立體聲的環繞餵法：`SL/SR = ±u`，完全反相。
在立體聲下那只是加寬，但反相的一對在上面的規則裡「和」恆為 0 —— 經過 HRTF
之後只剩純反相的 side。實測耳間相關性（IACC）：

| 素材 | P1 stereo | 修正前 binaural | 修正後 binaural |
|---|---|---|---|
| 置中 | +1.000 | +1.000 | +1.000 |
| 真實混音 amount=1.0 | +0.208 | **−0.453** | **+0.004** |
| 真實混音 amount=0.75 | +0.461 | +0.104 | +0.340 |
| 擴散 amount=1.0 | −0.436 | −0.570 | **−0.074** |

負的 IACC 聽起來是「在頭裡面、相位怪」，正好是頭外化的反面 —— 真實的雙耳
渲染不會讓一般音樂變成負相關。

修法是把環繞改成兩條**互不相關**的訊號（兩組固定隨機相位，預先算好和／差），
這也才是真實 5.1 環繞的樣子。**完全在 renderer 內部**，`_build_scene` 一行
沒動，所以不會撞到 P1 freeze。

擴散素材那一列值得記一下：修好之後 binaural（−0.074）**比 P1 的 stereo fold
（−0.436）更接近物理** —— 擴散場的 IACC 本來就該接近 0，而 stereo fold 只有
±u 這一條路可走。

代價是多一條 `surround_sum` 濾波器與兩次去相關乘法。`tests/test_hrtf.py` 有
一條「數濾波器條數」的測試，就是為了在這種時候強迫回頭重測預算 ——
它確實紅了一次，上面的量測表因此多了第二組數字。

#### 已知缺口（下一個切片）

1. **沒有耳廓 ⇒ 沒有可靠的前後區分，也沒有真正的頭外化。** 合成模型只給
   ITD 與 ILD。SADIE II（Apache-2.0，**不進版控**）是下一步，介面就是
   `HrtfFilters.from_ear_pairs`。UI 上不能拿合成模型冒充 HRTF 完成品。
2. **UI 尚未接線**，沒有使用者可見的開關。
3. **打包版不含此變更**：最近一次 `build_exe.py --verify` 的狀態是
   main + 取樣率修正，不含 HRTF renderer。
...
3. ✅ 人工 A/B（實機聽過並接受）
4. ✅ 早期反射（§9.8）
5. ⬜ **P1 freeze** —— 預算決定已定案（§9.9），打包版已補驗（§10.4）
6. 🔨 P2 HRTF —— renderer 骨架、預算重測、環繞反相修正都完成（§9.10）；
   還缺真人資料集與 UI 接線
...
| 打包版 | ✅ 2026-08-23 重驗（含 EQ／Spatial／反射）：167 MB／2702 檔、凍結環境 QML 載入成功、adapter 解析為 Windows。**不含 §9.10 的 HRTF renderer** |
