...
#: 量測截止達到分析鏈 Nyquist 的這個比例 ⇒ 量到的是分析鏈的天花板，不是內容。
#:
#: 只有引擎取樣率**低於**來源時才會用上：那種情況下高頻在進分析器之前就被
#: 重取樣濾掉了，再怎麼量都只會量到 Nyquist 附近。訂 0.90 是因為重取樣器的
#: 過渡帶通常落在 0.9×Nyquist 一帶。寧可多退回「無法判定」也不要拿分析鏈的
#: 限制去指控來源 —— 與 FAKE_LOSSLESS_CUTOFF_HZ 是同一個取捨。
ROLLOFF_ANALYSIS_LIMIT_RATIO: Final = 0.90
#: 量測受分析鏈限制時的標籤。語意上是 ROLLOFF_TIERS 之外的第六種結果：
#: 不是某個品質級距，而是「這次量測不能用來推估來源」。
ROLLOFF_LIMITED_LABEL: Final = "受引擎取樣率限制，無法推估來源"
...
#: 端點取樣率要連續被觀察到幾次，才通知引擎跟著切。
#:
#: 藍牙端點在連線、A2DP／HFP 切換、系統重設音訊堆疊的當下會短暫報出過渡
#: 格式。跟著過渡值切的代價是不對稱的：切下去的降取樣**不可逆**（高頻在
#: 第一次重取樣就沒了，之後切回來也救不回），而慢一拍才對齊只是多讓系統
#: 重取樣一個輪詢間隔。所以寧可等它穩定。
ENDPOINT_RATE_CONFIRM_POLLS: Final = 2
#: 跳轉快捷鍵的步進（秒）。
